...
hook.Add( "PlayerUse", "GuthSCP:PlayerUse", function( ply, ent )
    ...
    if GuthSCP.keycardAvailableClass[ent:GetClass()] then --  it's an keycard available class
        ...
        if (IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "weapon_access_card_chaos") then
            if (!ent.keycard_last_try) then
                ent.keycard_last_try = CurTime()
            end
    
            if (ent.keycard_last_try + process.config.IC_keycard_delay < CurTime()) then
                if (math.random(0, 1) == 1) then
                    --  good, he has passed all conditions, what haxor he is
                    ply:EmitSound( "guthen_scp/interact/KeycardUse1.ogg" )
                    ply:setBottomMessage( "Accès autorisé !" )

                    return
                else
                    ply:EmitSound( "guthen_scp/interact/KeycardUse2.ogg" )
                    ply:setBottomMessage( "Vous n'avez pas l'accréditation nécessaire !" )
                end

                ent.keycard_last_try = CurTime()
            else
                ply:setBottomMessage( "Attendez "..math.Round(ent.keycard_last_try + process.config.IC_keycard_delay -  CurTime()).." seconds.")
            end
        ...
        for _, weapon in pairs(ply:GetWeapons()) do
            local plyLVL = weapon.GuthSCPLVL

            if !isnumber(plyLVL) then continue end

            if plyLVL >= entLVL then
                --  good, he has passed all conditions, what haxor he is
                ply:EmitSound( "guthen_scp/interact/KeycardUse1.ogg" )
                ply:setBottomMessage( "Accès autorisé !" )
