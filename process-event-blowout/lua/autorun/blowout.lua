...
local BlowoutEnabled = CreateConVar( "event_process_actif", 0, { FCVAR_ARCHIVE }, "Active/desactive l'addon Blowout. La valeur peut etre 0 (= desactive) ou 1 (= active).")
local BlowoutInterval = CreateConVar( "blowout_interval", 0, { FCVAR_ARCHIVE }, "Le temps en secondes entre deux eruptions (de la phase de degats de l'eruption precedente a l'annonce de la suivante).")
local AnnounceDuration = CreateConVar( "blowout_announcing_duration", 60, { FCVAR_ARCHIVE }, "Le delai en secondes jusqu'a ce qu'une sequence d'eruption commence, apres qu'elle a ete annoncee.")
local DamageDuration = CreateConVar( "blowout_damage_duration", 180, { FCVAR_ARCHIVE }, "La duree en secondes, apres une eruption, pendant laquelle les Wiremod E2 ne peuvent pas etre utilises.")

local CTm = CurTime()
local AnnounceTime = CTm + math.max(BlowoutInterval:GetInt(), 10)
...
local function BlowoutReset( Plys, NPCs, E2s )
	if Plys > 0 then	--Reset the players' post processes
		for k, v in pairs(player.GetAll()) do
			...
			v:ChatPrint("Rétablissement des secteurs nécessaires finalisé...")
			BlowoutUnragdoll( v )
...
local function BlowoutThink()
	...
	if BlowoutEnabled:GetInt() <= 0 then
		if BlowoutLastEnabled == true then
			...
			print("[SCP-EVENT PROCESS] Addon has been disabled. ("..math.Round(CTm)..")")
	...
	elseif BlowoutLastEnabled == false then
		...
		print("[SCP-EVENT PROCESS] 	La prochaine explosion s'active dans "..math.Round(Next).." secondes. ("..math.Round(CTm)..")")
	...
	elseif BlowoutStarted then		--If the blowout has been triggered (warm-up phase completed)
		...
		if TimeBeforeHit <= 0 then	--Knock the players out
			...
			print("L'explosion a frappe ! Les E2 exposes sont desactives pour "..Time.." secondes. ("..math.Round(CTm)..")")
			...
			for k, v in pairs(player.GetAll()) do
				if !BadMoveTypes[v:GetMoveType()] and v:Health() > 0 then
					...
					if v.IsSafe then	--Player is safe
						...
						v.LastHealth = math.max(math.Round(v:Health() / 5), 1)
	...
	elseif CTm > AnnounceTime then		--If a blowout has been announced
		...
		elseif CTm > NextSound then
			if BlowoutStep == 0 then	--Initialise the announcing phase
				...
				print("[Blowout] Annoncer une eruption ! A partir de "..Duration.." secondes. ("..math.Round(CTm)..")")
...
local function BlowoutTriggerInstant( ply, cmd, args )
	...
	if BlowoutEnabled:GetInt() <= 0 then
		if IsValid(ply) then
			ply:ChatPrint("Avant d'activer la commande pour l'event vous devez activer le script de securite")
		else
			print("[SCP-EVENT PROCESS] L'addon est actuellement desactive, utilisez d'abord 'event_actif' pour l'activer.")
		end
		return
	end
	...
	if CurTime() > AnnounceTime or BlowoutStarted then
		if IsValid(ply) then
			ply:ChatPrint("Une sequence d'eruption est toujours active ! Attendez qu'il se termine en premier.")
		else
			print("[SCP-EVENT PROCESS] Une sequence d'eruption est toujours active ! Attendez qu'il se termine en premier.")	
		end
		return
	end
	...
	if IsValid(ply) then
		ply:ChatPrint("Frappes subies en cours ! Déploiement des système de sécurité.")
	else
		print("[SCP-EVENT PROCESS] Debut de la sequence d'eruption!")	
	end
...
local function BlowoutTriggerDelayed( ply, cmd, args )
	...
	if BlowoutEnabled:GetInt() <= 0 then
		if IsValid(ply) then
			ply:ChatPrint("L'addon Blowout est actuellement desactive, utilisez 'event_process_actif' pour l'activer en premier.")
		else
			print("[SCP-EVENT PROCESS] L'addon est actuellement desactive, utilisez d'abord 'event_process_actif' pour l'activer.")	
		end
		return
	end
	...
	if IsValid(ply) then
		ply:ChatPrint("Vous avez declenche une sequence d'eruption retardee!")
	end
...
concommand.Add("event_process_ic_bobardement_delay", BlowoutTriggerDelayed)
concommand.Add("event_process_ic_bombardement", BlowoutTriggerInstant)

BlowoutReset(1,1,1)
print("[SCP-EVENT] Serverside file loaded! ("..math.Round(CTm)..")")
